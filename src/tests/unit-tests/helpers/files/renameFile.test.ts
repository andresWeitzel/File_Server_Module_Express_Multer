...
//Vars
let checkFolderExistFromPathResult: any;
let msg: string;

describe("- checkFolderExistFromPath function (Unit Test)", () => {
  describe("1) Check cases for arguments.", () => {
    msg =
      "Should return a string value if a string type argument is passed (This function expects one argument of type string).";
    it(msg, async () => {
      checkFolderExistFromPathResult = await renameFile(MOCK_STRING_VALUE);
      expect(typeof checkFolderExistFromPathResult == "string").toBe(true);
    });
